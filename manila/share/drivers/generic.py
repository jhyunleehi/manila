...
class GenericShareDriver(driver.ExecuteMixin, driver.ShareDriver):
    ...
    def _create_share(self, context, share, snapshot, share_server=None):
        ...
        volume = self._allocate_container(self.admin_context, share, snapshot=snapshot)
        volume = self._attach_volume(self.admin_context, share, server_details['instance_id'], volume)
        if not snapshot:
            self._format_device(server_details, volume)

        self._mount_device(share, server_details, volume)
        export_locations = helper.create_exports(server_details, share['name'])
    ...
    def _format_device(self, server_details, volume):
        ...
        command = ['sudo', 'mkfs.%s' % self.configuration.share_volume_fstype,volume['mountpoint']]
        self._ssh_exec(server_details, command)
